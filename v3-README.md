...
- Cyan hand skeleton + gold fingertip reticle should track a real hand; a red-gold visor with cyan eye lenses should align to your face and disappear cleanly when your face leaves frame (hand tracking keeps working).
...
- GitHub Pages deployment is intentionally not configured yet — the parent repo's `/KB` folder contains the child's real name, school, and teacher names, so publishing needs a separate privacy decision (e.g. isolating this app into its own public-safe path/branch) before going live.
